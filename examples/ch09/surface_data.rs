...
fn create_quad(points: [Point3<f32>; 4], color: &ColormapInterpolator) -> Vec<VertexNC> {
    let normal = (points[2] - points[0]).cross(points[3] - points[1]).normalize();
    let vs: Vec<VertexNC> = points.iter()
        .map(|point| VertexNC::new(*point, normal, color.interpolate(point.y)))
        .collect();
...
pub fn parametric_surface_data(
    ...
    let mut vertices: Vec<VertexNC> = Vec::with_capacity(4 * (nu - 1) * (nv - 1));
    for i in 0..nu - 1 {
        for j in 0.. nv - 1 {
            let p0 = points[i][j];
            let p1 = points[i][j + 1];
